...
class Container<Trie<ValueT, option, CharT, SizeT> >
		: public MMap<Container<Trie<ValueT, option, CharT, SizeT> > >
{
public:
	...
	typedef ReverseIterator<Trie<ValueT, option, CharT, SizeT> > const_reverse_iterator;
...
public:
	...
	operator std::map<_Key, _Tp, _Compare, _Alloc>() const
	{
		...
		for (const_iterator it = begin(); it != end(); ++ it)
			result.insert(make_pair(key<key_type>(it), *it));
		return result;
	}
...
private:
	...
	friend class ReverseIterator<Trie<ValueT, option, CharT, SizeT> >;
};
...
public:
	...
	operator ()(const std::basic_string<_CharT, _Traits, _Alloc> &key) const
	{ return operator ()(key, range(key)); }
	template <class _Tp, class _Alloc>
	typename Container<ValueT>::const_reference
	operator ()(const std::vector<_Tp, _Alloc> &key) const
	{ return operator ()(key, range(key)); }
	template <class V, class S>
	typename Container<ValueT>::const_reference
	operator ()(const Vector<V, S> &key) const
	{ return operator ()(key, range(key)); }
	typename Container<ValueT>::const_reference operator ()(const char *key) const
	{ return operator ()(key, range(key)); }
	typename Container<ValueT>::const_reference operator ()(char *key) const
	{ return operator ()(key, range(key)); }
	template <class T>
	typename Container<ValueT>::const_reference operator ()(T *key) const
	{ return operator ()(key, range(key)); }
	...
	operator std::set<_Key, _Compare, _Alloc>() const
	{
		return std::set<_Key, _Compare, _Alloc>(
				m_container->m_maps.m_values.m_values.m_values1.begin() + begin().m_i,
				m_container->m_maps.m_values.m_values.m_values1.begin() + end().m_i);
	}
...
template <class ValueT, int option, class CharT, class SizeT>
Container<Trie<ValueT, option, CharT, SizeT> >::
Container(const char *filename, int prot, int flags)
		: MMap<Container<Trie<ValueT, option, CharT, SizeT> > >(filename, prot, flags)
{
	if (filename[0] == 0) throw int(-1);
		...
		OutIteratorT out, IteratorT begin, IteratorT end, void *skipLast)
{
	...
	entries.reserve(std::distance(begin, end) + 1);
		...
		OutIteratorT out, IteratorT begin, IteratorT end, void *skipLast)
{
	...
	size_t numTries  = std::distance(begin, end);
	size_t numValues = 0;
